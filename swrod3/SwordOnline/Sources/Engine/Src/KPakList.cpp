...
// 返回:	是否成功找到
//---------------------------------------------------------------------------
bool KPakList::FindElemFile(unsigned int uId, XPackElemFileRef& ElemRef)
{
	bool bFounded = false;
	for (int i = 0; i < m_nPakNumber; i++)
	{
...
bool KPakList::FindElemFile(const char* pszFileName, XPackElemFileRef& ElemRef)
{
	...
	if (pszFileName && pszFileName[0])
	{
		...
		unsigned int uId = FileNameToId(szPackName);
...
bool KPakList::FindElemFileEx(const char* pszFileName, XPackElemFileRef& ElemRef, int nExNum)
{
	bool bFounded = false;
	if (pszFileName && pszFileName[0])
	{
		...
		if(nExNum < 0)
			nExNum = 0;
		for (int i = nExNum; i < m_nPakNumber; i++)
		{
			if (m_PakFilePtrList[i]->FindElemFile(uId, ElemRef))
			{
				g_DebugLog("filefound int pak[%d][%s]",i, pszFileName);
				bFounded = true;
				break;
			}
		}
