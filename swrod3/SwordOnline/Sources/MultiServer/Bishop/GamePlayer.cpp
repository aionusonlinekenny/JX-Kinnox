...
#ifdef NO_PAYSYS
	char send[sizeof(KAccountUserReturnExt) + 1];
	send[0] = s2c_accountlogin;
	KAccountUserReturnExt* pReturn = (KAccountUserReturnExt*)&send[1];
	pReturn->Size = sizeof(KAccountUserReturnExt);
	pReturn->Type = AccountUserReturnEx;
	pReturn->Operate = m_lnIdentityID;
	pReturn->nLeftTime = pReturn->nExtPoint = 0;
	pReturn->nReturn = ACTION_SUCCESS;
	DispatchTaskForAccount( &send, sizeof(KAccountUserReturnExt) + 1 );
#else
	g_theSmartClient.Send( ( const void * )pData, datalength );
//	m_pAccSvrClient->SendPackToServer( ( const void * )pData, datalength );
#endif
...
#ifdef	CONSOLE_DEBUG
	cprintf( "CGamePlayer::VerifyAccount Failed! [name or pwd error]\n" );
#endif
...
#ifdef	CONSOLE_DEBUG
	cprintf( "CGamePlayer::VerifyAccount Failed! [Account Freeze]\n" );
#endif
