...
void KUiMsgCentrePad::DragWndPos(int nMoveOffset)
{
	if (nMoveOffset)
	{
		...
		x -= nMoveOffsetX;
		y -= nMoveOffsetY;
		m_MoveImg.SetPosition(x, y);	//按钮跟着窗口变化，所以要先变回原样
...
void KUiMsgCentrePad::DragWndSize(int nMoveOffset)
{
	...
	else
	{
		...
		x -= nMoveOffsetX;
		y -= nMoveOffsetY;
		m_SizeBtn.SetPosition(x, y);	//按钮跟着窗口变化，所以要先变回原样

		m_nSizeBtnDragOffsetY += nMoveOffsetY;

		if (ChanageHeight(m_nSizeBtnDragOffsetY, m_bSizeUp))	//改变Height成功后，就不用积累了
			m_nSizeBtnDragOffsetY = 0;
	}
}
