...
static int		l_ObjDataFileTotalDirColumn = 0;
static int		l_ObjDataFileTotalFrameColumn = 0;
...
#define ABOVE_HEAD_OBJ_ATLEAST_HIGH	80						//高空对象最低离地距离要求
...
#define	MAX_CHECK_TREE	10
static char		l_szTreeImgName[MAX_CHECK_TREE][48];
static int		l_nNumTree = 0;
static KIniFile	l_TreeListFile;

bool LoadTreeList()
{
	l_nNumTree = 0;
	memset(l_szTreeImgName, 0, sizeof(l_szTreeImgName));
	CString		File;
	File = "\\游戏资源\\美术图素\\城市\\两湖\\室外\\树木\\树木重心.txt";
	AppGetFullPath(File);
	l_TreeListFile.Clear();
	if (!l_TreeListFile.Load(File))
	{
		MessageBox(NULL, File, "打开文件错误。", MB_OK);
		return false;
	}
	l_TreeListFile.GetInteger("List", "Count", 0, &l_nNumTree);
	if (l_nNumTree > MAX_CHECK_TREE)
	{
		l_nNumTree = 0;
		MessageBox(NULL, File, "文件内列出的树数目过大，让程序改程序。", MB_OK);
		return false;
	}

	char	szIndex[8];
	for (int i = 0; i < l_nNumTree; i++)
	{
		itoa(i, szIndex, 10);
		l_TreeListFile.GetString("List", szIndex, "", l_szTreeImgName[i], sizeof(l_szTreeImgName[i]));
		if (l_szTreeImgName[i][0] == 0)
		{
			i --;
			l_nNumTree--;
		}
...
bool GetTreeCentre(const char* pszTreeImg, int  nFrame, int& nCentreX, int &nCentreY)
{
	if (pszTreeImg)
	{
		for (int i = 0; i < l_nNumTree; i++)
		{
			if (strstr(pszTreeImg, l_szTreeImgName[i]))
			{
				char	szSection[8], szIndex[8];
				itoa(i, szSection, 10);
				itoa(nFrame, szIndex, 10);
				nCentreX = -78;
				l_TreeListFile.GetInteger2(szSection, szIndex, &nCentreX, &nCentreY);
				return (nCentreX != -78);
			}
		}
	}
...
//	if (m_bPlane)
...

	{
		...
		l_ObjDataFileTotalDirColumn = l_ObjDataFile.FindColumn("ImageTotalDir");
		l_ObjDataFileTotalFrameColumn = l_ObjDataFile.FindColumn("ImageTotalFrame");
		l_ObjDataFileScriptColumn = l_ObjDataFile.FindColumn("ScriptName");
		l_ObjDataImgNameColomn = l_ObjDataFile.FindColumn("ImageName");
		if (l_ObjDataFileKindColumn < 1 ||	l_ObjDataFileTotalDirColumn < 1 ||
			l_ObjDataFileScriptColumn < 1 || l_ObjDataImgNameColomn < 1 ||
			l_ObjDataFileTotalFrameColumn < 1)
		{
...
bool CMapConvertDlg::LoadANpc(const char* pRootPath, struct KSPNpc* pNpc, class KIniFile* pFile, int nNpc, bool bServer)
{
	...
	NpcEvent.GetInteger("MAIN", "Golden", 0, &nValue);
	pNpc->bSpecialNpc = (nValue != 0);
	...
	if (!NpcEvent.GetInteger("1", "Kind", 3, &nValue))
	{
	...
	float	x, y, z;
	SM_GetPointFromFloatSting32(Buffer, x, y, z);
	pNpc->nPositionX = (int)x;
	pNpc->nPositionY = (int)y;
	pNpc->cReserved[0] = pNpc->cReserved[1] = 
		pNpc->cReserved[2] = 0;
	return true;
}
...
bool CMapConvertDlg::LoadAObj(const char* pRootPath, struct KSPObj* pObj, class KIniFile* pFile, int nObj, KBuildinObj* pBio, bool bServer)
{
	...
	pFile->GetInteger(ObjSection, "id", 0, &nValue);
	int nFrame = 0;
	pFile->GetInteger(ObjSection, "iframe", 0, &nFrame);
	...
	int nTotalFrame, nTotalDir;

	l_ObjDataFile.GetInteger(pObj->nTemplateID + 1, l_ObjDataFileTotalFrameColumn,
			0, &nTotalFrame);
	l_ObjDataFile.GetInteger(pObj->nTemplateID + 1, l_ObjDataFileTotalDirColumn,
			0, &nTotalDir);

	if (nTotalDir && (nTotalFrame / nTotalDir))
	{
		nFrame = nFrame % nTotalFrame;
		pObj->nDir = nFrame / (nTotalFrame / nTotalDir);
	}
	else
		pObj->nDir = 0;
...
//		pObject->nOrder = nValue;
		...
		(pObject->Props & SPBIO_P_BLOCK_LIGHT_MASK) == SPBIO_P_BLOCK_LIGHT_BOTTOM)
	{
		if (strstr(pObject->szImage, "\\游戏资源\\美术图素\\城市\\两湖\\室外\\树木"))
		{
			...
			if ((pObject->Props & SPBIO_P_SORTMANNER_MASK) == SPBIO_P_SORTMANNER_TREE)
			{
				if (GetTreeCentre(pObject->szImage, pObject->nFrame, nCentreX, nCentreY))
				{
					str = pObject->szImage;
					AppGetFullPath(str);
					KSprite	spr;
					...
					{
						nCentreX -= pFrame->OffsetX;
						nCentreY -= pFrame->OffsetY;
						if (nCentreX < 0)
							nCentreX = 0;
						if (nCentreY < 0)
							nCentreY = 0;
