...
//	char *temp_buffer;							
//	char *temp_ptr;
//	unsigned long temp_size;					//临时缓冲区的大小
public:
	ZCache(unsigned long size);
	virtual ~ZCache();
	char *getNode(unsigned char key_size, unsigned long size);					//创建指定大小的节点
	char *searchNode(const char *key, unsigned char key_size);					//搜索指定的节点
//	char *getMemory(unsigned long size);										//获取一段临时使用的缓冲区
//	void freeMemory();															//释放所有的缓冲区
};
