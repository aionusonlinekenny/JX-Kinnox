...
void CD3DApplication::SetPos(DWORD x, DWORD y)
{
	m_x = x;
	m_y = y;
}
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DApplication::Create( HINSTANCE hInstance, HWND hWnd, ULONG  sx, ULONG  sy, bool bFullScreen)
{
    HRESULT hr;
...
HRESULT CD3DApplication::BuildDeviceList()
{
    ...
    for( UINT iAdapter = 0; iAdapter < m_pD3D->GetAdapterCount(); iAdapter++ )
    {
        ...
        for( UINT iMode = 0; iMode < dwNumAdapterModes; iMode++ )
        {
            ...
            if( DisplayMode.Width  != m_sx || DisplayMode.Height != m_sy )
                continue;
...
//							MessageBox(m_hWnd,"use hw pure device",NULL,MB_OK);
						}
					}
                    ...
                    if ( FALSE == bFormatConfirmed[f] )
                    {
						...
						{
                            bFormatConfirmed[f] = TRUE;
//							MessageBox(m_hWnd,"use hw hardware_vp device",NULL,MB_OK);
						}
					}
                    ...
                    if ( FALSE == bFormatConfirmed[f] )
                    {
						...
						{
                            bFormatConfirmed[f] = TRUE;
//							MessageBox(m_hWnd,"use hw mix_vp device",NULL,MB_OK);
						}
					}
                }

                // Confirm the device/format for SW vertex processing
                if( FALSE == bFormatConfirmed[f] || 1)
                {
					...
					{						
                        bFormatConfirmed[f] = TRUE;
//						MessageBox(m_hWnd,"use hw soft_vp device",NULL,MB_OK);
					}
            ...
            {
                if( pDevice->modes[m].Width==m_sx && pDevice->modes[m].Height==m_sy )
                {
    ...
    for( DWORD a=0; a<m_dwNumAdapters; a++ )
    {
        ...
        {
            if( m_bWindowed == FALSE || m_Adapters[a].devices[d].bWindowed == m_bWindowed)
            {
				m_Adapters[a].devices[d].bWindowed = m_bWindowed;
        ...
        case WM_EXITSIZEMOVE:
            ...
            if( m_bActive && m_bWindowed )
            {
                    ...
                    m_rcWindowClient.bottom - m_rcWindowClient.top)
                {
					...
					m_d3dpp.BackBufferWidth  = m_sx;
					m_d3dpp.BackBufferHeight = m_sy;
...
#endif
            D3DUtil_SetDeviceCursor( m_pd3dDevice, hCursor, 0 );
            m_pd3dDevice->ShowCursor( TRUE );
        }
...
HRESULT CD3DApplication::Resize3DEnvironment()
{
	...
	{
		//ASSERT(0);
		OutputDebugString("Error when InvalidateDeviceObjects\n");
		return hr;
	}
	...
	{
		OutputDebugString("Error when Reset\n");
		return hr;
	}
    ...
    if( FAILED(hr) )
	{
		OutputDebugString("Error when RestoreDeviceObjects\n");
        return hr;
	}
