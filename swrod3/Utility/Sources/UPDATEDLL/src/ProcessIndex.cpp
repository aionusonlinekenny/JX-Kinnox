...
int CProcessIndex::InitVersionInfo()
{
    ...
    for (i = 0; i < defVersion_ValueSize; i++)
    {
        sprintf(szKeyName, "AppVersion%d", i);
        ::GetPrivateProfileString("Version", szKeyName, "", 
            m_Version_Value[i].szVersionStr, MAX_PATH, m_szTxtFileName
        );
        if (*(m_Version_Value[i].szVersionStr) == '\0')
            break;

		strupr(m_Version_Value[i].szVersionStr);
        sprintf(szKeyName, "Version%d", i);
        GetPrivateProfileString("Version", szKeyName, "0", szValue, 70, m_szTxtFileName);
        m_Version_Value[i].nVersion = _atoi64(szValue);

        m_nVersion_ValueCount++;
    }
